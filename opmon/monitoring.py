    ...
    def validate(self) -> None:
        ...
        if callable(self.before_execute_callback):
            ...
            self.before_execute_callback(
                ...
                annotations={
                    "slug": self.slug,
                    "type": "validate_metrics_query_original",
            ...
            self.before_execute_callback(
                ...
                annotations={
                    "slug": self.slug,
                    "type": "validate_metrics_query_dryrun",
                    "submission_date": self.config.project.start_date,
                },
            )
        ...
        if callable(self.before_execute_callback):
            self.before_execute_callback(
                ...
                annotations={
                    "slug": self.slug,
                    "type": "validate_statistics_query_original",
                    "submission_date": self.config.project.start_date,
                },
            )
        ...
        if callable(self.before_execute_callback):
            self.before_execute_callback(
                ...
                annotations={
                    "slug": self.slug,
                    "type": "validate_statistics_query_dryrun",
                    "submission_date": self.config.project.start_date,
                },
            )
        ...
        if total_alerts > 0:
            ...
            if callable(self.before_execute_callback):
                self.before_execute_callback(
                    ...
                    annotations={
                        "slug": self.slug,
                        "type": "validate_alerts_query_original",
                        "submission_date": self.config.project.start_date,
                    },
                )
            ...
            if callable(self.before_execute_callback):
                self.before_execute_callback(
                    ...
                    annotations={
                        "slug": self.slug,
                        "type": "validate_alerts_query_dryrun",
                        "submission_date": self.config.project.start_date,
                    },
                )
